...
function Categories() {
    ...
    useEffect(()=>{
        fetch(`https://dummyjson.com/products/category/${categoryId}`).then((a)=>a.json()).then((b)=>setData(b.products))
    },[])
  return (
    <div className='container mx-auto py-3'>
      <h1 className='text-2xl font-bold uppercase'>{categoryId}</h1>
      <section id="Projects" className="w-fit mx-auto grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 justify-items-center justify-center gap-y-20 gap-x-14 mt-10 mb-5">
 
 {data.map((a)=>(
     <div  className="w-72 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl">
     <Link to={`/detail/${a.id}`}>
       <img src={a.thumbnail} alt="Product" className="h-80 w-72 object-cover rounded-t-xl" />
